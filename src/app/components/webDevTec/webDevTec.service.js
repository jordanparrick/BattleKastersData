(function() {
  ...
  function data($http, $q, baseUrl) {
    ...
    var landInclude = [{relation: 'wisdom'}];
    ...
    var service = {
      ...
      loadLand: function (landId) {
        return findOne('/api/lands', landId, landInclude);
      },
      ...
      loadLands: function () {
        return _http({method: "GET", url: '/api/lands', params: {filter: {include: landInclude}}});
      },
      loadSpells: function () {
      ...
      addMonsterToBattle: function (battleId, monster) {
        return _http({method: "POST", url: "/api/battles/" + battleId + "/monsters", data: monster})
      },
      ...
      searchSpellsByProfile: function (email) {
        var config = {
      ...
      importWisdom: function (Blobs) {
        var config = {url: '/api/wisdom/import', method: "POST", data: {files: Blobs}};
        $("#upload-in-progress").show();
        return _http(config).then(function(res){
          $("#upload-in-progress").hide();
          return res;
      ...
      delete: function (item, endpoint, list) {
      ...
      loadNewUsers: function(startDate, endDate, stats){
        var config = {method: "GET", url:'/api/profiles', params: {filter: {where: {createdDate: {between: [startDate, endDate]}}, include: 'installations'}}};
        return _http(config).then(function(res){
      ...
      loadQuestStats: function(startDate, endDate, stats){
        var config = {method: "GET", url:'/api/gameStateQuests', params: {filter: {where: {createdDate: {between: [startDate, endDate]}}, include: 'quest'} }};
      ...
      getStonefireClosers: function(startDate, endDate, stats){
        var config = {method: 'GET', url:'/api/gameStates', params: {filter:{where: {createdDate: {between: [startDate, endDate]}, stopTime: {neq: null}}, include: 'profile'}}};
        return _http(config).then(function(res){
          stats.stonefireClosers = res.data;

        })
      },
      gatherAnalytics: function(startDate, endDate){
        var deferred = $q.defer();
        startDate = startDate ? new moment(startDate).toISOString() : new moment().startOf('day').subtract(1, 'd').toISOString();
        endDate = endDate ? new moment(endDate).toISOString() : new moment().toISOString();
        //startDate = startDate + "Z";
        var calls = [];
        var stats = {};
        calls.push(service.getStonefireClosers(startDate, endDate, stats));
        calls.push(service.loadNewUsers(startDate, endDate, stats));
        calls.push(service.loadQuestStats(startDate, endDate, stats));
    ...
    this.gatherAnalytics = service.gatherAnalytics;
